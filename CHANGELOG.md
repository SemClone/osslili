...
- **The notice that says which member of a shared-text family a file carries is now read** (Issue #144). Seven texts on the SPDX list are shared by licences that oblige different things, so an exact match on the text names a family rather than a licence. What separates the members is written down, just not in the licence file, and the scan now reads it
  ...
  | the project writes | reported |
  |---|---|
  | Exhibit B in a source header | `MPL-2.0-no-copyleft-exception` |
  | a Reserved Font Name after the copyright | `OFL-1.1-RFN` |
  | `with the Invariant Sections being ...` in a document | `GFDL-1.3-invariants-only` |
  | `with no Invariant Sections` in a document | `GFDL-1.3-no-invariants-only` |
  | `SPDX-License-Identifier: CAL-1.0-Combined-Work-Exception` in a source file | `CAL-1.0-Combined-Work-Exception` |

  - Such a record carries the match type `exact_hash_named_by_notice`, a confidence of 0.95, no `ambiguous_with`, and a `resolved_by` naming the file the notice was read in and the words it said. The finding is an inference across two files, so a reader who doubts it can go and look
  - **The notice is recognised where a project applies it, not where the licence defines it.** Every distinguishing phrase also occurs in the licence that asks for it: "Incompatible With Secondary Licenses" five times in MPL-2.0, "Invariant Section" twenty times in GFDL-1.3, and both texts close with a template inviting an author to copy the phrase out. A reader that did not know the difference would mark every MPL project as carrying Exhibit B, which is the direction that costs something — it says code cannot be relicensed under the GPL when it can. A file carrying the licence's own heading for a notice is carrying the template, and is not read for it
  - SIL's unfilled `with Reserved Font Name <Reserved Font Name>` and the GFDL's `LIST THEIR TITLES` are placeholders rather than declarations, and settle nothing
  - The cheap check for whether a file is worth reading closely accepts exactly the separators the notice readers accept. Written out twice they drifted, and an Exhibit B notice wrapped across a `;` or `!` comment was never looked at
  - **This is cross-file.** A notice in `src/main.c` changes the identifier reported for `LICENSE`, which nothing in osslili did before. The notices are read as the files are read, so no scan is read twice to answer a question seven licence families ask
  - **The notice has to open a line**, after nothing but indentation and a comment marker, which is the rule already applied to "Licensed under the Apache License". A CONTRIBUTING.md explaining that a file saying `This Source Code Form is "Incompatible With Secondary Licenses"` cannot be combined with GPL code marked its whole project as carrying the exception it was telling contributors not to add
  - **A licence printed inside a document is skipped where it is printed, not wherever it appears.** The GFDL tells an author to include a copy of the licence in the document, so a manual states its grant at the top and appends the licence below; a guard that dismissed any file containing the addendum dismissed exactly the documents this reads
  - A project can contradict itself two ways, and both count. A document stating its invariant sections beside another declaring `GFDL-1.3-no-invariants-only` outright is as much a disagreement as two notices are, and checking only notice against notice let the notice answer for the licence file they share
  - **The project's own identifiers can disagree too.** One source file saying `CAL-1.0` and another saying `CAL-1.0-Combined-Work-Exception` is a disagreement, and leaving the member the text itself chose out of the count made it look like a single clear answer and granted the exception
  - A disagreement elsewhere is a reason not to answer for *other* files, not a reason to leave a file contradicting itself. Two manuals that disagree and each append the licence keep their shared COPYING ambiguous, and each is still led by what it says about itself
  - Nothing is settled unless exactly one member is named. Silence leaves the ambiguity 1.9.1 reports: a plain `OFL.txt` with no reserved name is a file that did not say, not evidence of `OFL-1.1-no-RFN`. Two notices that contradict each other leave it too, whether they are in two files or one, because that disagreement is itself the finding
  - A grant already worked out is kept. The GFDL family splits on invariant sections and on `-only` against `-or-later` at once, and a document naming its Invariant Sections has said nothing about later versions
  - **A manifest is never where a notice is applied.** It describes a package rather than carrying its terms, and a `pyproject.toml` long description showing contributors the Exhibit B header settled the LICENSE beside it as carrying the exception it was quoting
  - **Each notice counts only where the licence asking for it says to write it.** Exhibit B says what it applies to in its own first words, "This Source Code Form is ...", so a CONTRIBUTING.md showing the header in a fenced code block — to tell contributors *not* to add it — is not attaching it, and neither is a NOTICE listing it for contributors to copy. A reserved font name is written after the copyright statement, so the same words in a README credit somebody else's font. Invariant Sections are the exception that proves it: the GFDL's notice belongs in the document, and documentation is exactly where it counts
  - **A bundled third-party notice does not settle the project's own licence** (issue #78), and the project's notices do not settle a dependency's. A vendored font's `THIRD_PARTY_NOTICES.txt` names its own reserved font name, and that is the dependency's paperwork
  - A licence named in a sentence is still a mention (#117). A README saying "the vendored font is licensed under `OFL-1.1-RFN`" is a credit, and reading it as a declaration let it rewrite the project's own licence
  - **Exhibit B is attached per file, so a project can carry both members.** Where one source file carries the notice and another does not, the project has not settled the question, and a notice answers for files other than its own only once every header has been read. True whether the notice-bearing file states an identifier of its own or not: with no identifier there is nothing to qualify, and the disagreement never showed up as two declarations. A manifest is not one of those files — it summarises the package rather than marking a source form, and `"license": "MPL-2.0"` beside a file carrying Exhibit B is the ordinary shape of a Mozilla project
  - A notice fills in what an identifier left out, so it qualifies `MPL-2.0` and has nothing to say about `MPL-2.0-no-copyleft-exception`. A file that already names a member has answered, and reading a notice as a qualification anyway let it quietly replace the identifier the file wrote
  - Qualifying a header changes what it states, and what the files are held to state is read again afterwards. A file carrying `SPDX-License-Identifier: MPL-2.0` and Exhibit B states the exception member once the two have been read together, and counting its first answer left it looking as though it disagreed with another file declaring that same member outright
  - Not being able to place the licence in the file is not the same as the file having no preface, and answering both the same way discarded a real declaration above a lightly edited licence text
  - Mozilla's standard header states `SPDX-License-Identifier: MPL-2.0` and attaches Exhibit B beneath it, and the two are one declaration. Read as two they contradicted each other, and the header went on declaring the family at confidence 1.0 beside the LICENSE just corrected to the member — so every consumer that takes the most confident record, `get_primary_license` and the KissBOM writer among them, reported the licence the correction was made to replace. The notice now qualifies the identifier in its own file, and only there: Exhibit B is attached per file, so a manifest naming the family is a different claim about a different scope
  - The file credited in `resolved_by` is the same one every run. What each file yielded is collected as the threads finish, so taking the first file to declare a member took whichever worker won the race (#122)
  - **Known limit**: a shared text recognised only by similarity rather than by an exact hash is reported without the `ambiguous_with` 1.9.1 adds, so a notice cannot be applied to it. Annotating the fuzzy tiers is the other half of #142 and is separate work
  - **Known limit**: a licence referenced through PEP 639's `license = {file = "OFL.txt"}` is attributed to the manifest, so a notice in the referenced file is not associated with it and the record stays ambiguous. That is the honest answer rather than a wrong one, and closing it is separate work
  - In an archive, the file a notice was read in is named by its path inside it, as `source_file` already is (#121). It was being reported as the extraction directory, which is different every run and gone by the time anyone opens the report
  - An identifier settles the rest of the family as a notice does. A manifest declaring `MPL-2.0-no-copyleft-exception` settled the LICENSE while a README quoting the same shared text went on scoring against the plain family: one licence, two readings, in one scan
  - **Nothing answers for a file other than its own until the whole scan has been checked for disagreement.** Two documents that state their licence and disagree about their invariant sections each settled their own record and then answered for the COPYING they shared, in whatever order the threads finished, so two identical scans gave the shared file different answers
  - **A reserved font name settles the file it is written in and no other.** Said of the identifier rather than of the notice, so `SPDX-License-Identifier: OFL-1.1-RFN` in one licence file does not reach the vendored font beside it either. Unlike Exhibit B it needs no cross-file reading at all, because it is on the licence file's own copyright line — and a project may hold two OFL files, its own font and a vendored one, where one naming a reserved name says nothing whatever about the other
  - A licence text does not stop being one for having a sentence in front of it. A LICENSE opening "This project is licensed as follows" no longer hashes, so the CAL's printed identifiers stayed declarations at 1.0 — and then suppressed the 0.999 similarity reading of the same file as a guess at what the file had already said, so the quoted identifier was silencing the reading that contradicted it. A close enough score counts too, in a licence file or a document: something whose whole subject is the licence
  - Only a whole-file match counts as a licence quoting itself. A source file is not compared whole: a licence block is cut out of it first, and a block matching exactly says nothing about the rest of the file. One carrying a short licence verbatim and declaring `SPDX-License-Identifier: 0BSD OR Apache-2.0` beside it lost the Apache half of its own declaration
  - A header needs no licence file beside it. A source tree with no LICENSE in it, or one file named on the command line, has no ambiguous record to hang the reading off, and Mozilla's header is exactly that case
  - Exhibit B and the CAL's identifier are in source files, so those two need `--deep`. A Reserved Font Name is in the licence file itself and Invariant Sections are in documentation, so those two are read by a default scan

### Fixed
- **A licence that prints an SPDX identifier in its own text was read as declaring it** (Issue #144). The CAL prints both of its identifiers in its section on how to mark a file, so every project under `CAL-1.0` reported `CAL-1.0-Combined-Work-Exception` at confidence 1.0 from its LICENSE alone: an exception nobody had granted, read out of the instructions for granting it. The Community Specification did the same with `CC-BY-4.0`

  - An identifier printed inside a licence text that was recognised outright is the licence quoting itself, and it is discounted before the scan works out what corroborates what. FSL-1.1-ALv2 prints the Apache boilerplate as the licence a work converts to, and the doomed Apache identifier was keeping an Apache keyword match in the same file alive: the scan reported `Apache-2.0` on the strength of two findings that were both the FSL text
  - Sound because of what an exact match means: the hash is taken over the file's whole text, so a file that matched carries the canonical text and nothing else, and anything printed in it is SPDX's words
  - **Asked of the identifier, not of the file.** A file may carry a licence text *and* declare something: a LICENSE opening `SPDX-License-Identifier: MIT OR 0BSD` above the MIT text is offering a choice, and discounting every identifier in it lost the 0BSD half. What is discounted is an identifier the matched licence prints in its own words — fourteen texts on the SPDX list name another licence that way, GPL-3.0 naming the AGPL among them — so this takes nothing else with it
  - **A project may state what its licence also prints, and where it says so is what tells them apart.** A LICENSE opening `SPDX-License-Identifier: CAL-1.0-Combined-Work-Exception` above the CAL text is granting the exception the CAL explains how to grant, and what is read is the run of text before the licence begins. Counting occurrences instead could not separate them: the same identifier reaches the reader as a canonical id in one place and as prose in another, and is counted under different names, so a real declaration looked like the licence's own. Only files that matched a licence text and carry one of the fourteen identifiers such a text prints are read again, which is a handful in any scan
  - **A notice is a condition of a grant, so the grant has to be there.** A CONTRIBUTING.md telling authors not to write "with no Invariant Sections" carries the words without granting anything, and settled the whole project as having none. Naming the licence is not enough either — "do not add the phrase ... to GFDL manuals" names it while granting nothing — so what a document applying it carries is the addendum's own wording: permission granted, under the terms of
  - Which identifiers a licence prints is worked out by running the tag reader over its bundled text, so the answer is the same question the scan asks of a file. Looking only for `SPDX-License-Identifier:` lines missed FSL-1.1-ALv2, which names the licence a work converts to as "Licensed under the Apache License, Version 2.0"
  - Asked of every file that matched, not only the ones named like a licence. A `README.md` carrying the CAL in full quotes its identifiers just as a LICENSE does, and declared the combined-work exception at 1.0 and then used that to settle its own record
  - A declaration in any other file still counts, which is what lets a source header settle the CAL family above
