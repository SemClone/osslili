...
- **A file that states its licence no longer gets a near neighbour reported beside it.** With every licence text bundled, a `LICENSE` stating `Python-2.0` also scored 0.961 against `Python-2.0.1`, the same licence one revision apart, and both were reported. A similarity score on a file that spoke for itself is a guess about text already answered; this is what #108 settled for normalisation, applied to the tiers
